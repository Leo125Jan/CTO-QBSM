...
class UAV():
	...
	def __init__(self):
		...
		self.cmd_vel = Twist()
		self.px4 = Px4Controller("uav1")

		# Publisher & Subscriber
		self.imu_sub = rospy.Subscriber("/uav1/mavros/imu/data", Imu, self.imu_callback, queue_size = 20, buff_size = 52428800)
		self.odom_sub = rospy.Subscriber('/gazebo/model_states', ModelStates, self.odom_callback, queue_size = 20, buff_size = 52428800)

		self.fov_sub_0 = rospy.Subscriber("/uav0/fov", Float64MultiArray, self.fov_0_callback, queue_size = 20, buff_size = 52428800)
		self.fov_sub_2 = rospy.Subscriber("/uav2/fov", Float64MultiArray, self.fov_2_callback, queue_size = 20, buff_size = 52428800)
		self.cluster_dist_sub_0 = rospy.Subscriber("/uav0/cluster_dist", Float64MultiArray,\
												self.cluster_dist_0_callback, queue_size = 20, buff_size = 52428800)
		...
		self.cluster_teammate_sub_0 = rospy.Subscriber("/uav0/cluster_teammate", Float64MultiArray,\
												self.cluster_teammate_0_callback, queue_size = 20, buff_size = 52428800)
		...
		self.neighor_0_fov = np.zeros(size)
		self.neighor_2_fov = np.zeros(size)
		self.neighor_0_cluster_dist = np.array([100, 100, 100])
		self.neighor_2_cluster_dist = np.array([100, 100, 100])
		self.neighor_0_cluster_teammate = np.array([None, None])
	...
	def fov_0_callback(self, msg):

		self.neighor_0_fov = msg.data

	def fov_2_callback(self, msg):

		self.neighor_2_fov = msg.data

	def cluster_dist_0_callback(self, msg):

		self.neighor_0_cluster_dist = msg.data

	def cluster_dist_2_callback(self, msg):

		self.neighor_2_cluster_dist = msg.data

	def cluster_teammate_0_callback(self, msg):

		self.neighor_0_cluster_teammate = msg.data
	...
	def Collect_Data(self):
		...
		# Neighbors States
		self.neighbors_fov = np.array([self.neighor_0_fov, self.neighor_2_fov])

		self.neighor_0_cluster_dist = np.array(self.neighor_0_cluster_dist)
		self.neighor_2_cluster_dist = np.array(self.neighor_2_cluster_dist)
		self.neighor_0_cluster_teammate = np.array(self.neighor_0_cluster_teammate)
		self.neighor_2_cluster_teammate = np.array(self.neighor_2_cluster_teammate)

		self.neighbors_cluster_dist = np.array(\
		[self.neighor_0_cluster_dist if np.all(self.neighor_0_cluster_dist != None) else [None, None, None],\
		...
		[self.neighor_0_cluster_teammate if np.all(self.neighor_0_cluster_teammate != None) else [None, None, None],\
		self.neighor_2_cluster_teammate if np.all(self.neighor_2_cluster_teammate != None) else [None, None, None]])

		self.neighbors_position = np.array([[self.P1[0], self.P1[1]], [self.P3[0], self.P3[1]]])
	...
	def	controller(self, dx, dp, step):
		...
		u_des = np.array([1.0*( (P3[0] - P2[0]) - 1.0 + (P1[0] - P2[0]) - 2.0 + (P4[0] - P2[0]) - 1.0 + P4v[0]),\
			1.0*( (P3[1] - P2[1]) - sqrt(3) + (P1[1] - P2[1]) + 0.0 + (P4[1] - P2[1]) - 0.5*sqrt(3) + P4v[1]),\
			0.7 - P2[2]])
		...
		self.cmd_vel.linear.z = 0.5 - self.P2[2]
...
if __name__ == '__main__':
	...
	try:
		...
		ptz_2 = PTZcon(camera1, map_size, grid_size)
		uav_2 = UAV()

		while uav_2.P2 is None:
